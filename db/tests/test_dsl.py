...
--------------------
lease_status (expiring + expired leases)
  expiring: end_date >= today AND end_date <= today + params.warn_days
            urgency: high if days_until_end <= 30, medium otherwise
  expired:  end_date < today AND unit.active_lease_count == 0
            urgency: high
  category: leasing
...
def _audit(db, check=None, **check_cfgs):
    ...
    all_checks = {
        "lease_status":          {"enabled": False, "interval_hours": 1, "warn_days": 60},
...
class TestExpiringLeases:
    ...
    def test_lease_expiring_within_default_warn_window_triggers(self, db):
        ...
        n = _audit(db, "lease_status", warn_days=EXPIRY_WARN_DAYS)
        assert n == 1
    ...
    def test_already_expired_lease_not_counted_as_expiring(self, db):
        """end_date < today → creates 'Expired lease' task, not 'Lease expiring'."""
        p = _prop(db)
        u = _unit(db, p)
        ...
        n = _audit(db, "lease_status", warn_days=EXPIRY_WARN_DAYS)
        assert n == 1
        assert not any("Lease expiring" in s for s in _subjects(db))
    ...
    def test_custom_warn_days_30_triggers_within_30(self, db):
        ...
        n = _audit(db, "lease_status", warn_days=30)
        assert n == 1
    ...
    def test_warn_days_90_catches_lease_at_89_days(self, db):
        ...
        n = _audit(db, "lease_status", warn_days=90)
        assert n == 1
    ...
    def test_multiple_expiring_leases_produce_separate_tasks(self, db):
        ...
        n = _audit(db, "lease_status", warn_days=60)
        assert n == 3
...
class TestExpiredLeases:
    ...
    def test_expired_lease_no_replacement_triggers(self, db):
        ...
        n = _audit(db, "lease_status")
        assert n == 1
        assert any("Expired lease" in s for s in _subjects(db))

    def test_active_lease_excluded_by_filter(self, db):
        """end_date well past warn_days → not expiring or expired."""
        p = _prop(db)
        u = _unit(db, p)
        t = _tenant(db, phone="555-4002")
        _lease(db, p, u, t, end=TODAY + timedelta(days=365))
        n = _audit(db, "lease_status")
        assert n == 0

    def test_lease_ending_today_not_expired(self, db):
        """end_date == today is active, not expired (but may be expiring)."""
        p = _prop(db)
        u = _unit(db, p)
        t = _tenant(db, phone="555-4003")
        _lease(db, p, u, t, end=TODAY)
        n = _audit(db, "lease_status")
        # Lease ending today triggers expiring check (within warn_days window)
        assert n == 1
        assert any("Lease expiring" in s for s in _subjects(db))
        assert not any("Expired lease" in s for s in _subjects(db))
...
class TestCheckFiltering:
    ...
    def test_check_name_filter_runs_only_that_check(self, db):
        """check_name='lease_status' must not run incomplete_properties."""
        _prop(db, city=None)
        p = _prop(db)
        u = _unit(db, p)
        t = _tenant(db, phone="555-6001")
        _lease(db, p, u, t, end=TODAY + timedelta(days=20))
        n = run_data_audit(db, check_name="lease_status")
        subjects = _subjects(db)
        assert any("Lease expiring" in s for s in subjects)
    ...
    def test_warn_days_param_respected_via_config(self, db):
        ...
        cfg = {
            "checks": {
                "lease_status": {"enabled": True, "interval_hours": 1, "warn_days": 30},
