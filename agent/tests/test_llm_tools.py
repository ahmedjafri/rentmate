...
# ---------------------------------------------------------------------------
from agent.agent_query import _validate_sql
...
def test_chat_with_agent_propagates_simulation_context():
    from agent.client import chat_with_agent
    ...
    try:
        with patch("agent.loop.litellm.acompletion", side_effect=fake_acompletion), \
             patch("agent.client.agent_registry.build_system_prompt_bundle", return_value={
...
def test_chat_with_agent_logs_tool_traces_with_conversation_id():
    ...
    with (
        patch("agent.loop.litellm.acompletion", side_effect=fake_acompletion),
        patch("agent.loop.dispatch", side_effect=fake_dispatch),
        patch("agent.client.agent_registry.build_system_prompt_bundle", return_value={
            "system_prompt": "system",
            "memory_context": "",
            "parts": [],
        }),
        patch("agent.client.log_trace", side_effect=lambda *args, **kwargs: logged.append({"args": args, "kwargs": kwargs})),
    ):
...
def test_load_onboarding_prompt_only_for_active_chat_sessions():
    from agent.client import _load_onboarding_prompt

    fake_db = MagicMock()
    fake_session_local = MagicMock(return_value=fake_db)

    with (
        patch("agent.client._read_onboarding_prompt", return_value="## Onboarding Mode\nUse onboarding rules."),
        patch("db.session.SessionLocal", fake_session_local),
        patch("services.settings_service.get_onboarding_state", return_value={"status": "active"}),
    ):
...
def test_load_onboarding_prompt_skips_non_chat_or_inactive_sessions():
    ...
    with (
        patch("agent.client._read_onboarding_prompt", return_value="## Onboarding Mode\nUse onboarding rules."),
        patch("db.session.SessionLocal", fake_session_local),
        patch("services.settings_service.get_onboarding_state", return_value={"status": "completed"}),
    ):
...
def test_local_fallback_retries_when_reply_claims_document_without_tool_call():
    from agent.client import _local_fallback
    from agent.tools import pending_suggestion_messages
    ...
    with patch("agent.client.chat_with_agent", side_effect=fake_chat_with_agent), \
         patch("agent.client.set_fallback_request_context", return_value=object()), \
         patch("agent.client.reset_fallback_request_context"), \
         patch("agent.client.resolve_account_id", return_value=1), \
         patch("agent.client.resolve_org_id", return_value=1):
...
def test_local_fallback_blocks_switch_to_different_mutating_tool_after_failure():
    from agent.client import _local_fallback, current_completed_tools, current_failed_tools
    from agent.tools import pending_suggestion_messages
...
from agent.agent_action import _queue_action
