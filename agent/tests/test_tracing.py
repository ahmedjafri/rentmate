...
from agent.tracing import _orphan_warned, log_trace
from db.models import AgentTrace
from integrations.local_auth import reset_fallback_request_context, set_fallback_request_context
...
def test_log_trace_dropped_outside_active_run(db, caplog):
    ...
    token = set_fallback_request_context(account_id=1, org_id=1)
    try:
        with caplog.at_level(logging.WARNING, logger="agent.tracing"):
