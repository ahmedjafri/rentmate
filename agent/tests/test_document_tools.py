...
from agent.tools import AnalyzeDocumentTool, ReadDocumentTool
...
def test_read_document_returns_structured_failure_and_trace(db, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="rentmate.agent.tools"), _test_run():
...
def test_analyze_document_logs_exception_details(db, caplog):
    ...
    with caplog.at_level(logging.ERROR, logger="rentmate.agent.tools"), \
         patch("agent.document_processor.process_document", side_effect=RuntimeError("processor offline")), \
         _test_run():
...
def test_analyze_document_full_tool_path_persists_and_reads_extracted_fields(db):
    ...
    with (
        ...
        patch("agent.document_processor._get_session_factory", return_value=lambda: db),
        patch("integrations.wire.storage_backend.download", return_value=_blank_pdf_bytes()),
