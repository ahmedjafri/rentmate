...
from agent.generated_documents import RenderedDocument
...
def test_update_task_progress_marks_steps_done_and_allows_close(db):
    ...
    with patch("db.session.SessionLocal.session_factory", return_value=db), \
        ...
        closed = json.loads(_run_tool(
            __import__("agent.tools", fromlist=["CloseTaskTool"]).CloseTaskTool(),
            task_id=str(task.id),
        ))
...
def test_create_document_tool_creates_document_and_queues_action_card(db):
    ...
    try:
        with patch("db.session.SessionLocal.session_factory", return_value=db), \
             patch.object(db, "close", lambda: None), \
             patch("integrations.wire.storage_backend.upload", new_callable=AsyncMock) as upload_mock, \
             patch("agent.generated_documents.render_document", return_value=RenderedDocument(
...
def test_remember_about_entity_private_uses_current_account(db):
    ...
    with patch("db.session.SessionLocal.session_factory", return_value=db), \
         patch.object(db, "close", lambda: None), \
         patch("agent.tools.memory.resolve_account_id", return_value=property_owner.id):
        payload = json.loads(_run_tool(
            RememberAboutEntityTool(),
...
_READ_WRITE_TOOL_CASES = [
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["CreatePropertyTool"]).CreatePropertyTool(),
        {"address": "123 Sim St", "property_type": "single_family"},
        id="create_property",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["CreateTenantTool"]).CreateTenantTool(),
        {"first_name": "Sim", "last_name": "Tenant"},
        id="create_tenant",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["CreateVendorTool"]).CreateVendorTool(),
        {"name": "Sim Plumber", "phone": "+15550009999", "vendor_type": "Plumber"},
        id="create_vendor",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["CreateSuggestionTool"]).CreateSuggestionTool(),
        {"title": "Sim suggestion", "body": "body", "suggestion_type": "compliance", "risk_score": 3},
        id="create_suggestion",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["MessageExternalPersonTool"]).MessageExternalPersonTool(),
        {"entity_id": "sim-entity", "entity_type": "tenant", "draft_message": "Sim check-in"},
        id="message_person",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["AddTaskNoteTool"]).AddTaskNoteTool(),
        {"task_id": "9999", "note": "Sim progress note"},
        id="add_task_note",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["RememberAboutEntityTool"]).RememberAboutEntityTool(),
        {
            "entity_type": "property",
    ...
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["EditMemoryTool"]).EditMemoryTool(),
        {"entity_type": "property", "entity_id": "sim-prop", "new_context": "Sim context"},
        id="edit_memory",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["ProposeTaskTool"]).ProposeTaskTool(),
        {"title": "Sim task", "category": "maintenance"},
        id="propose_task",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["CloseTaskTool"]).CloseTaskTool(),
        {"task_id": "sim-task", "reason": "Sim close"},
        id="close_task",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["UpdateTaskProgressTool"]).UpdateTaskProgressTool(),
        {"task_id": "sim-task", "step_key": "step-1", "status": "done"},
        id="update_task_progress",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["RecordTaskReviewTool"]).RecordTaskReviewTool(),
        {"task_id": "1", "status": "on_track", "summary": "Sim review."},
        id="record_task_review",
    ),
    pytest.param(
        lambda: __import__("agent.tools", fromlist=["AskManagerTool"]).AskManagerTool(),
...
def test_read_write_tools_are_blackholed_in_simulation(db, tool_factory, args):
    ...
    """
    from agent.tools._common import ToolMode, simulation_actions
...
def test_message_person_inherits_task_id_from_active_conversation(db):
    ...
    from agent.tools._common import active_conversation_id
    from db.models import Conversation, ConversationType
...
def test_record_task_review_writes_columns_and_trace(db):
    ...
    """
    from agent.runs import start_run
    from agent.tools import RecordTaskReviewTool
...
def test_ask_manager_posts_to_task_ai_conversation(db):
    ...
    """
    from agent.tools import AskManagerTool
