...
from backends.runtime_config import DeploymentMode, DocGenBackend, RuntimeConfig, load_runtime_config
from llm.doc_gen_loader import load_doc_gen_module

runtime = load_doc_gen_module("runtime")
...
def test_get_doc_gen_client_chooses_grpc_when_configured():
    ...
    assert isinstance(runtime.get_doc_gen_client(config), runtime.GrpcDocGenClient)


def test_local_doc_gen_client_renders_via_subprocess(monkeypatch, tmp_path):
    client = runtime.LocalDocGenClient()
    ...
    monkeypatch.setattr(runtime.tempfile, "NamedTemporaryFile", fake_named_tempfile)
    monkeypatch.setattr(runtime.subprocess, "run", fake_run)

    rendered = client.render_document(runtime.RenderDocumentRequest(html_document="<h1>hello</h1>"))
...
def test_local_doc_gen_client_surfaces_subprocess_errors(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError, match="document renderer subprocess failed: BrowserType.launch: TargetClosedError"):
        client.render_document(runtime.RenderDocumentRequest(html_document="<h1>hello</h1>"))
...
def test_local_doc_gen_client_includes_subprocess_context_on_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(runtime.tempfile, "NamedTemporaryFile", fake_named_tempfile)
    monkeypatch.setattr(runtime.subprocess, "run", fake_run)
    monkeypatch.setattr(runtime.os, "getcwd", lambda: "/tmp/rentmate-api")
    monkeypatch.setattr(runtime, "_doc_gen_env_snapshot", lambda: {"HOME": "/home/tester", "TMPDIR": "/tmp"})
...
def test_doc_gen_grpc_roundtrip():
    class FakeLocalClient:
        def render_document(self, request: runtime.RenderDocumentRequest) -> runtime.RenderDocumentResult:
            assert request.html_document == "<h1>hello</h1>"
            return runtime.RenderDocumentResult(pdf_bytes=b"%PDF-1.4 grpc", renderer="weasyprint")
    ...
    server, port = runtime.build_doc_gen_grpc_server(config=config, local_client=FakeLocalClient())
    server.start()
    try:
        client = runtime.GrpcDocGenClient(replace(config, doc_gen_grpc_target=f"127.0.0.1:{port}"))
        rendered = client.render_document(runtime.RenderDocumentRequest(html_document="<h1>hello</h1>"))
    finally:
...
def test_doc_gen_grpc_reports_remote_failures():
    ...
    server, port = runtime.build_doc_gen_grpc_server(config=config, local_client=FailingLocalClient())
    server.start()
    try:
        client = runtime.GrpcDocGenClient(replace(config, doc_gen_grpc_target=f"127.0.0.1:{port}"))
        with pytest.raises(grpc.RpcError, match="renderer crashed"):
            client.render_document(runtime.RenderDocumentRequest(html_document="<h1>boom</h1>"))
    finally:
        server.stop(grace=None)
