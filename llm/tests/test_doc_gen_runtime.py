...
from llm.doc_gen_runtime import (
    GrpcDocGenClient,
    LocalDocGenClient,
...
def test_local_doc_gen_client_renders_via_subprocess(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("llm.doc_gen_runtime.tempfile.NamedTemporaryFile", fake_named_tempfile)
    monkeypatch.setattr("llm.doc_gen_runtime.subprocess.run", fake_run)
...
def test_local_doc_gen_client_includes_subprocess_context_on_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("llm.doc_gen_runtime.tempfile.NamedTemporaryFile", fake_named_tempfile)
    monkeypatch.setattr("llm.doc_gen_runtime.subprocess.run", fake_run)
    monkeypatch.setattr("llm.doc_gen_runtime.os.getcwd", lambda: "/tmp/rentmate-api")
    monkeypatch.setattr("llm.doc_gen_runtime._doc_gen_env_snapshot", lambda: {"HOME": "/home/tester", "TMPDIR": "/tmp"})
